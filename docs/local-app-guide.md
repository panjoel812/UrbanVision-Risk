...
One upload automatically starts transform-consensus detection and the independent local crack proposal in parallel. The green proposal immediately produces a pixel-only geometry draft. Sensitivity disagreement becomes a bounded ranked queue. Version 4.5 adds a 640 × 360 synchronized loupe for the active review zone. Its aspect-preserving source viewport expands around the original-resolution bounding box, reports the effective zoom, and maps every pointer coordinate back into source-image pixels. Brush and eraser strokes share the same stroke history, undo path, mask raster, and final audit comparison as the main canvas. Starting a loupe edit also marks that ranked target inspected. Calibration-point editing remains restricted to the full-image canvas.

上传一次会并行自动启动变换共识检测和本地裂缝建议，绿色底稿随后生成像素几何草稿；灵敏度分歧形成有上限的排序队列。v4.5 为当前复核区域增加 640 × 360 同步放大窗：保持宽高比的源图视口围绕原分辨率边界框扩展，显示实际倍率，并把每个指针坐标映射回原图像素。放大窗与主画布共用笔迹历史、撤销、掩膜栅格和最终审计比较；开始在放大窗修订时也会自动把当前目标记为已检查。标定点仍只允许在完整主图上操作。
