...
One upload automatically starts transform-consensus detection and the independent local crack proposal in parallel. The green proposal immediately produces a pixel-only geometry draft. Sensitivity disagreement becomes a bounded ranked queue. The 640 × 360 synchronized loupe maps every pointer coordinate back into source-image pixels and shares the main canvas mask and undo path. Version 4.6 adds a structured operator disposition to every target: accepted as proposed, false positive removed, missed crack added, or deferred for follow-up, plus a bounded optional note. Loupe brush and eraser actions automatically select the matching correction disposition. The service rejects unknown target IDs, unsupported categories, duplicate decisions, and decisions that were not also marked inspected. Calibration-point editing remains restricted to the full-image canvas.

上传一次会并行自动启动变换共识检测和本地裂缝建议，绿色底稿随后生成像素几何草稿；灵敏度分歧形成有上限的排序队列。640 × 360 同步放大窗把指针坐标映射回原图像素，并与主画布共用掩膜和撤销路径。v4.6 为每个目标增加结构化处置：接受候选、误检已删除、漏检已补画或暂缓跟进，并可填写限长备注。放大窗画笔和橡皮会自动选择相应修正处置。服务端拒绝未知编号、不支持的类别、重复决策，以及没有同时标为已检查的决策。标定点仍只允许在完整主图操作。
