...
One upload automatically starts transform-consensus detection and the independent local crack proposal in parallel. The green proposal immediately produces a pixel-only geometry draft. Sensitivity disagreement becomes a bounded ranked queue, while the synchronized loupe maps edits into source-image pixels. Structured dispositions remain server-validated. Version 4.7 additionally creates `active-learning-feedback.zip` whenever a saved human review has at least one disposition. Each ranked item exports source, proposal-mask, final-mask, and disagreement-layer PNGs plus a manifest. Crops include 15% context with an eight-pixel minimum, are capped at 512,000 exported pixels, and use nearest-neighbour mask resizing. The manifest stores every member digest, the source digest, the immutable measurement digest, disposition, priority, original crop coordinates, scale, privacy boundary, and intended use.

上传一次会并行自动启动变换共识检测和本地裂缝建议，绿色底稿生成像素几何草稿；灵敏度分歧形成有上限的排序队列，同步放大窗把修订映射到原图像素，结构化处置仍由服务端校验。v4.7 会在保存至少一项处置后额外生成 `active-learning-feedback.zip`。每个排序项导出原图、候选掩膜、最终掩膜和分歧层 PNG 及清单。裁剪增加 15% 上下文且至少 8 像素，导出面积限制为 512,000 像素，掩膜使用最近邻缩放。清单记录所有成员摘要、源图摘要、不可变量测摘要、处置、优先级、原始裁剪坐标、缩放比例、隐私边界和预期用途。
