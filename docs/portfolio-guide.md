...
## What makes v4.3 different / v4.3 的差异化

Many public road-damage projects combine RDD2022, one YOLO checkpoint, and a Streamlit or Gradio upload page. UrbanVision-Risk v4.3 demonstrates linked detection, reliability, metrology, and human-review systems. The reliability layer performs deterministic data preparation, held-out evaluation, transform-consensus inference, safety gating, explainable scoring, immutable provenance, and active-learning review. The metrology layer adds skeleton graphs, topology, physical-plane calibration, automatic field fiducials, width distributions, sensitivity analysis, and a reproducible field experiment. The review layer generates an automatic pixel draft, exposes sensitivity-disagreement hotspots, preserves every proposal artifact, and measures the exact human correction rather than silently treating automation as ground truth.

许多公开道路检测项目只是 RDD2022、一个 YOLO 权重和 Streamlit/Gradio 上传页面。UrbanVision-Risk v4.3 把检测、可靠性、量测和人工复核连接成完整系统：可靠性层负责确定性数据准备、留出评估、变换共识推理、安全门控、可解释评分、不可变来源记录和主动学习；量测层加入骨架图、拓扑、真实平面标定、自动现场标记识别、宽度分布、敏感性分析和可复现现场实验；复核层自动生成像素草稿、暴露灵敏度分歧热点、保留每份建议证据，并精确量化人工修改，而不是静默把自动结果当作真值。
...
- Engineered a human-in-the-loop review layer that perturbs proposal sensitivity at three levels, visualizes disagreement as targeted hotspots, and audits automatic-versus-reviewed masks with immutable hashes, pixel deltas, and IoU.
...
- 设计人机协同复核层，在三个灵敏度下扰动候选算法，将分歧可视化为定向复核热点，并用不可变摘要、像素增删量与 IoU 审计自动掩膜和人工结果。
